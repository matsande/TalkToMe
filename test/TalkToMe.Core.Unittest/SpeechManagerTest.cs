...
namespace TalkToMe.Core.Unittest
{
    public class SpeechManagerTest
    {
        ...
        public void ShouldInvokeSpeakCommand()
        {
            ...
            var speechManager = new SpeechManager(clipboardMonitor, voiceFactory, keyMonitor, configStore, config, pluginManager);
        ...
        [Fact]
        public void ShouldInvokeSpeakCommandWithPluginConvertedText()
        {
            var hotKeys = new Dictionary<KeyInfo, CommandType>
            {
                { new KeyInfo(Keys.A, Keys.Shift), CommandType.Speak }
            ...
            var textPlugin = Substitute.For<ITextFormatPlugin>();
            textPlugin.FormatText(Arg.Any<string>()).Returns(a => "Test" + a[0]);

            var pluginManager = Substitute.For<IPluginManager>();
            pluginManager.Plugins.Returns(new List<ITextFormatPlugin> { textPlugin });
            ...
            voice.Received().Speak("Test" + text);
        }
